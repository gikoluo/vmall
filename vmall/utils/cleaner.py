...
def safe_html(html):
    ...
    for tag in soup.findAll():
        if tag.name.lower() in blacklist:
            ...
            #tag.attrs = [(k, safe_css(k, v)) for k,v in tag.attrs if _attr_name_whitelisted(k)]

            tag.attrs = { k.lower(): v for (k,v) in tag.attrs.items() if k.lower() in attr_name_whitelist }
...
#     return attr_name.lower() in ["href", "src"]
